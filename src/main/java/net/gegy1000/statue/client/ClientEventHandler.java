        ...
        if (state.getBlock() instanceof StatueBlock) {
            ...
            if (tile instanceof StatueBlockEntity) {
                ...
                if (model instanceof OutlineRenderer) {
                    ...
                    renderOutline(event, partialTicks, player, pos, tile, entity, model, texture, facing, outlineRenderer);
                }
            }
        }
    }

    private static void renderOutline(DrawBlockHighlightEvent event, float partialTicks, EntityPlayer player, BlockPos pos, TileEntity tile, StatueBlockEntity entity, ModelBase model, ResourceLocation texture, EnumFacing facing, OutlineRenderer outlineRenderer) {
        ...
        int light = player.world.getCombinedLight(tile.getPos(), 0);
